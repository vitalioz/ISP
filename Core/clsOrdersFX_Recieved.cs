...
namespace Core
{
    public class clsOrdersFX_Recieved
    {
        ...
        public void GetRecord()
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add(new SqlParameter("@Table", "CommandsFX_Recieved"));
        ...
        public void GetList()
        {
            ...
            try
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter("@Table", "CommandsFX_Recieved"));
                cmd.Parameters.Add(new SqlParameter("@Col", "CommandFX_ID"));
                cmd.Parameters.Add(new SqlParameter("@Value", this._iCommandFX_ID));
                cmd.Parameters.Add(new SqlParameter("@Order", "ID DESC"));
                drList = cmd.ExecuteReader();
                while (drList.Read())
                {
                    _dtList.Rows.Add(drList["ID"], drList["CommandFX_ID"], drList["DateIns"], drList["Method_ID"], drList["FilePath"], drList["FileName"]);
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, Global.AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
            finally { conn.Close(); }
        ...
        public int InsertRecord()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("sp_InsertCommandsFXRecieved", conn))
                {
        ...
        public void EditRecord()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("sp_EditCommandsFXRecieved", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void DeleteRecord()
        {
            try
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@Table", SqlDbType.NVarChar, 100).Value = "CommandsFX_Recieved";
