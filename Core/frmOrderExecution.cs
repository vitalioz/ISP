...
namespace Core
{
    public struct ExecCommandClient
    {
        public int Command_ID;
        public decimal Quantity;
        public decimal Koef;
    }
    public partial class frmOrderExecution : Form
    {
        int i, iRec_ID, iLastAktion, iCommandType_ID, iCustody_ID, iDepository_ID, iEditable, iContract_ID, iFeesCalcMode, iStatus, iRightsLevel,
            ...
            iProduct_ID, iProductCategory_ID, lblProductStockExchange_ID;
        ...
        private void frmOrderExecution_Load(object sender, EventArgs e)
        {
            ...
            if (iCommandType_ID == 2) {             // 2 - ExecutionOrder, 3 - BulkOrder
            ...
            if (iRec_ID != 0) {
                ...
                lblProductStockExchange_ID = klsOrder.ProductStockExchange_ID;
                ...
                {
                    lblSumPrice.Text = (decRealAmount / decRealQuantity).ToString("0.00##");
            ...
            picFixOut.Visible = false;
            if (Convert.ToDateTime(dSend.Value).Date == Convert.ToDateTime("1900/01/01") && (iServiceProvider_ID == 19 || iServiceProvider_ID == 20)) picFixOut.Visible = true;
        ...
        private void tslCancel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(sMessage, Global.AppTitle, MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes) { 
                ...
                if (bFIX_A && (iServiceProvider_ID == 19 || iServiceProvider_ID == 20))                             // 20 - BNP ARBITRAGE     
                {
                    ...
                    NewOrders = new clsNewOrders();
                    NewOrders.InsType = 2;                                             // 2 - Cancel Entoli
                    NewOrders.SendFlag = 0;
                    NewOrders.CurrentTimestamp = DateTime.Now;
                    //NewOrders.SequenceNumber = 0;
                    NewOrders.MsgType = "F";
                    NewOrders.PossDupFlag = char.Parse("N");
                    NewOrders.ClOrdID = iRec_ID.ToString() + "C";
                    NewOrders.SecurityID = lnkISIN.Text;
                    NewOrders.IDSource = char.Parse("4");                             // 4 - ISIN Number
                    NewOrders.Account = klsOrder.Client_ID == 3227 ? "782937P" : "782061A";
                    NewOrders.Side = char.Parse(klsOrder.Aktion.ToString());
                    NewOrders.OrderQty = Convert.ToInt32(txtQuantity.Text);
                    //NewOrders.ClientID = "HELLASFIN";
                    NewOrders.OrigClOrdID = iRec_ID.ToString();
        ...
        private void tsbSave_Click(object sender, EventArgs e)
        {
            ...
            if (bContinue)
            {
                ...
                if (iRec_ID == 0)
                {
                    ...
                    klsOrder.ExecutionStockExchange_ID = 0;
                ...
                else
                {
                    ...
                    if (fgExecutions.Rows.Count > 1) klsOrder.ExecutionStockExchange_ID = Convert.ToInt32(fgExecutions[1, "StockExchange_ID"] + "");
                    else klsOrder.ExecutionStockExchange_ID = 0;
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            ...
            txtSendMinute.Enabled = true;
            txtSendSecond.Enabled = true;
            if (!bFIX_A && (iServiceProvider_ID == 19 || iServiceProvider_ID == 20)) picFixOut.Visible = true;                       // 20 - BNP ARBITRAGE
            dSend.Focus();
        ...
        private void picFixOut_Click(object sender, EventArgs e)
        {
            i = 0;
            i = Global.AddNewOrders(klsOrder.Client_ID, iRec_ID, klsOrder.Aktion.ToString(), iServiceProvider_ID, lnkISIN.Text, lstType.SelectedIndex, 
                                    lblCurr.Text, Convert.ToDecimal(txtPrice.Text), Convert.ToDecimal(txtQuantity.Text), chkBestExecution.Checked ? 1 : 0,
                                    lblProductStockExchange_ID);
            if (i > 0)
            {
                Orders = new clsOrdersSecurity();
                Orders.Record_ID = iRec_ID;
                Orders.GetRecord();
                Orders.FIX_A = 0;
                Orders.EditRecord();
                picFixOut.Visible = false;
        ...
        protected void ucPS_TextChange(object sender, EventArgs e)
        {
            ...
            lblProductStockExchange_ID = stProduct.StockExchange_ID;
