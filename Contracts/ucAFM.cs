...
namespace Contracts
{
    public partial class ucAFM : UserControl
    {
        ...
        private void ucAFM_Load(object sender, EventArgs e)
        {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (sDescription.Length > 0)
            {
                ...
                AFM_Data = JsonConvert.DeserializeObject<AFMData>(tokens[0]);
                lblOldAFM.Text = AFM_Data.old_afm;

                AFM_Data = JsonConvert.DeserializeObject<AFMData>(tokens[1]);
                txtNewAFM.Text = AFM_Data.new_afm;

                string[] bokens = tokens[2].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    AFM_Data = JsonConvert.DeserializeObject<AFMData>(bokens[i]);
                    if (AFM_Data.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + AFM_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = AFM_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + AFM_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void txtNewAFM_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
            CreateDescription();
        }
        ...
        {
            if (txtNewAFM.Text.Trim() == "") lblStatus_Grp2.Visible = true;
            ...
            sTemp = sTemp + "~";
            sDescription = "{'old_afm' : '" + lblOldAFM.Text + "'}~{'new_afm' : '" + txtNewAFM.Text + "'}~" + sTemp;
        ...
        {
            public string old_afm { get; set; }
            public string new_afm { get; set; }
