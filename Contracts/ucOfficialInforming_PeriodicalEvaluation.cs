...
namespace Contracts
{
    public partial class ucOfficialInforming_PeriodicalEvaluation : UserControl
    {
        DataColumn dtCol;
        DataRow dtRow;
        DataTable dtInform;
        int i, iClient_ID;
        string sTemp, sClientName, sRecipientName, sDate, sThema, sContractTitle, sConnectionMethod, sConnectionData, sAttachedFiles, sOldCode, sBody, sPeriod, sFileFullName;
        bool bCheckList;
        DateTime dStart, dFinish;
        Global.ContractData stContractData;
        clsContracts_PeriodicalEvaluation Contracts_PeriodicalEvaluation = new clsContracts_PeriodicalEvaluation();
        public ucOfficialInforming_PeriodicalEvaluation()
        {
            InitializeComponent();
        }

        private void ucOfficialInforming_PeriodicalEvaluation_Load(object sender, EventArgs e)
        {
            bCheckList = false;
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            sPeriod = "01/01/" + sTemp + " - " + "31/12/" + sTemp;

            //---- Define List ---------------------
            Contracts_PeriodicalEvaluation = new clsContracts_PeriodicalEvaluation();
            Contracts_PeriodicalEvaluation.Year = Convert.ToInt32(cmbYear.Text);
            Contracts_PeriodicalEvaluation.GetList();
        ...
        private void ShowList()
        {
            if (bCheckList)
            {
                ...
                foreach (DataRow dtRow in Contracts_PeriodicalEvaluation.List.Rows)
                {
                    if (lblCode.Text.Trim() == "" || dtRow["Code"].ToString() == lblCode.Text)
                    {
                        i = i + 1;
                        ...
                        sClientName = "";
                        if (Convert.ToInt32(dtRow["ClientTipos"]) == 1)
                        {                           // Atomiki
                            sClientName = (dtRow["Surname"] + " " + dtRow["Firstname"]).Trim();
                            sRecipientName = (dtRow["Surname"] + " " + dtRow["Firstname"]).Trim();
                        }
                        else
                        {
                            sClientName = dtRow["Surname"] + "";
                        ...
                        sDate = dtRow["DateSent"] + "";
                        if (sDate.Length > 0) sDate = Convert.ToDateTime(sDate).ToString("dd/MM/yyyy");

                        fgList.AddItem(false + "\t" + i + "\t" + sClientName + "\t" + dtRow["ContractTitle"] + "\t" + dtRow["ProviderTitle"] + "\t" +
                                       dtRow["Code"] + "\t" + dtRow["Portfolio"] + "\t" + sDate + "\t" + sConnectionMethod + "\t" + sConnectionData + "\t" +
                                       dtRow["FileName"] + "\t" + dtRow["ID"] + "\t" + dtRow["Client_ID"] + "\t" + dtRow["ConnectionMethod_ID"] + "\t" +
                                       dtRow["ContractTipos"] + "\t" + dtRow["Contract_ID"] + "\t" + dtRow["Contract_Details_ID"] + "\t" + dtRow["Contract_Packages_ID"]);
                    }
                }
                fgList.Redraw = true;
            }
        ...
        private void tsbSend_Click(object sender, EventArgs e)
        {
            ...
            sOldCode = "~~~";
            sThema = "Περιοδική αξιολόγηση καταλληλότητας";
                ...
                if (Convert.ToBoolean(fgList[i, 0]))
                {
                    ...
                    if (Convert.ToInt32(fgList[i, "ConnectionMethod_ID"]) == 1)                                     // 1 - e-mail
                    {
                        ...
                        if (sOldCode != (fgList[i, "Code"] + ""))                     // if it's a new code write into Informings table record that will be send
                        {
                            ...
                            iRec_ID = Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 5, 8, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "Client_Data"] + "",
                                               "", "Επίσημη Ενημέρωση Πελατών", sBody, fgList[i, "FileName"] + "", "", "", 0, 0, "");                        // 5 - e-mail 
                        }
                        ...
                        {
                            iRec_ID = Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 5, 8, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "Client_Data"] + "",
                                               "", "Επίσημη Ενημέρωση Πελατών", sBody, fgList[i, "FileName"] + "", "", DateTime.Now.ToString(), 1, 1, "");   // 5 - e-mail  
                        }
                    ...
                    if (Convert.ToInt32(fgList[i, "ConnectionMethod_ID"]) == 2)                  // 2 - post
                    {
                        ...
                        dtRow["f5"] = "ΘΕΜΑ: Περιοδική αξιολόγηση καταλληλότητας";
                        sTemp = "Στο πλαίσιο που ορίζει ο νόμος 4514/2018, σας αποστέλλουμε την περιοδική αξιολόγηση της καταλληλότητας των χρηματοπιστωτικών μέσων που περιέχονται στο χαρτοφυλάκιό σας. " + "\n\n";
                        ...
                        {
                            sTemp = fgList[i, 5] + "";
                            Global.DMS_PrintFile("Customers/" + sTemp.Replace(".", "_") + "/Invoices".ToString(), fgList[i, "FileName"].ToString());
                        }
                        sTemp = fgList[i, "ContractTitle"] + "";

                        Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 8, 8, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]),
                                                  fgList[i, "Client_Data"] + "", "", sThema, "", (fgList[i, "FileName"] + ""), sAttachedFiles, DateTime.Now.ToString(), 1, 1, "");     // 8 - post                      
                    }

                    clsContracts_PeriodicalEvaluation Contracts_PeriodicalEvaluation = new clsContracts_PeriodicalEvaluation();
                    Contracts_PeriodicalEvaluation.Record_ID = Convert.ToInt32(fgList[i, "ID"]);
                    Contracts_PeriodicalEvaluation.GetRecord();
                    Contracts_PeriodicalEvaluation.DateSent = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
                    Contracts_PeriodicalEvaluation.EditRecord();
        ...
        private void tsbPrint_Click(object sender, EventArgs e)
        {
            ...
            for (i = 1; i <= fgList.Rows.Count - 1; i++)
            {
                ...
                dtRow["f4"] = fgList[i, "ProviderTitle"];
                dtRow["f5"] = fgList[i, "Code"];
                dtRow["f6"] = fgList[i, "Portfolio"];
                dtRow["f7"] = fgList[i, "DateSent"];
                dtRow["f8"] = fgList[i, "ConnectionMethod"];
                dtRow["f9"] = fgList[i, "Client_Data"];
                dtRow["f10"] = fgList[i, "FileName"];
                dtInform.Rows.Add(dtRow);
            }

            frmReports locReports = new frmReports();
            locReports.Params = "" + "~" + cmbYear.Text + "~" + Global.UserName + "~" + Global.CompanyName + "~";
            locReports.ReportID = 25;
            locReports.ShowResult = dtInform;
            locReports.Text = "Επίσημη Ενημέρωση Πελατών";
            locReports.Show();
        }
        ...
        private string PerformanceFeesEmailBody(int iInvoice)
        {
            ...
            sBody = "<html xmlns='http://www.w3.org/1999/xhtml'><head><title></title><meta http-equiv='Content-Type' content='text/html; charset=utf-8' />" +
                     ...
                     "<center> ΘΕΜΑ: Περιοδική αξιολόγηση καταλληλότητας " + sPeriod + "</center>" + "<br/><br/><br/><br/>" +
                     "Αγαπητέ πελάτη,<br/><br/>" +
                     "Στο πλαίσιο που ορίζει ο νόμος 4514/2018, σας αποστέλλουμε την περιοδική αξιολόγηση της καταλληλότητας των χρηματοπιστωτικών μέσων που περιέχονται στο χαρτοφυλάκιό σας. <br/><br/><br/>" +
