...
namespace Contracts
{
    public partial class ucOfficialInforming_CustodyFees : UserControl
    {
        DataColumn dtCol;
        DataRow dtRow;
        DataTable dtInform;
        int i, j, iClient_ID, iCF_ID, iCF_Quart;
...
#pragma warning disable CS0414 // The field 'ucOfficialInforming_CustodyFees.bCheckList' is assigned but its value is never used
        bool bCheckList;
#pragma warning restore CS0414 // The field 'ucOfficialInforming_CustodyFees.bCheckList' is assigned but its value is never used
        Global.ContractData stContractData;
        public ucOfficialInforming_CustodyFees()
        {
            InitializeComponent();
        ...
        private void ucOfficialInforming_CustodyFees_Load(object sender, EventArgs e)
        {
            bCheckList = false;
        ...
        private void DefineCommandsList()
        {
            ...
            if (Convert.ToInt32(cmbProviders.SelectedValue) != 0)
            {
                ...
                klsCustodyFees_Title.SC_ID = Convert.ToInt32(cmbProviders.SelectedValue);
                klsCustodyFees_Title.CF_Year = Convert.ToInt32(cmbYear.Text);
                klsCustodyFees_Title.CF_Quart = iCF_Quart;
                klsCustodyFees_Title.GetRecord_Title();
                iCF_ID = klsCustodyFees_Title.Record_ID;

                fgList.Redraw = false;
                fgList.Rows.Count = 1;
                //-------------- Define CustodyFees_Recs List ------------------
                clsCustodyFees_Recs CustodyFees_Recs = new clsCustodyFees_Recs();
                CustodyFees_Recs.CT_ID = iCF_ID;
                ...
                if (Convert.ToBoolean(fgList[i, 0]))
                {
                    ...
                    if (Convert.ToInt32(fgList[i, "ConnectionMethod"]) == 1)                                     // 1 - e-mail
                    {
                        ...
                        if (sOldCode != (fgList[i, "Code"] + ""))                     // if it's a new code write into Informings table record that will be send
                        {
                            ...
                            iRec_ID = Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 5, 10, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "ClientData"] + "",
                    ...
                    if (Convert.ToInt32(fgList[i, "ConnectionMethod"]) == 2)                  // 2 - post
                    {
                        ...
                        dtRow["f5"] = "ΘΕΜΑ: ΑΜΟΙΒΗ ΘΕΜΑΤΟΦΥΛΑΚΗΣ";
                        ...
                        locReports.Show();
                        if ((fgList[i, "Invoice_File"] + "") != "") Global.DMS_PrintFile("Customers/" + sContractTitle + "/Invoices".ToString(), fgList[i, "Invoice_File"].ToString());
                        if ((fgList[i, "Statement_File"] + "") != "") Global.DMS_PrintFile("Customers/" + sContractTitle + "/Informing".ToString(), fgList[i, "Statement_File"].ToString());
                        if ((fgList[i, "Misc_File"] + "") != "") Global.DMS_PrintFile("Customers/" + sContractTitle + "/Informing".ToString(), fgList[i, "Misc_File"].ToString());
                        sTemp = fgList[i, "ContractTitle"] + "";

                        Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 8, 10, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]),
                                                  fgList[i, "ClientData"] + "", "", "Επίσημη Ενημέρωση Πελατών", "", "", sAttachedFiles, DateTime.Now.ToString(), 1, 1, "");           // 8 - post                      
                    }

                    clsCustodyFees_Recs CustodyFees_Recs = new clsCustodyFees_Recs();
                    CustodyFees_Recs.Record_ID = Convert.ToInt32(fgList[i, "ID"]);
                    CustodyFees_Recs.GetRecord();
                    CustodyFees_Recs.OfficialInformingDate = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
                    CustodyFees_Recs.EditRecord();
        ...
        private void mnuViewInvoice_Click_1(object sender, EventArgs e)
        {
            sTemp = fgList[fgList.Row, "ContractTitle"] + "";
            if (sTemp.Length > 0 && ((fgList[fgList.Row, "Invoice_File"] + "") != "")) Global.DMS_ShowFile("Customers/" + sTemp.Replace(".", "_") + "/Invoices", fgList[fgList.Row, "Invoice_File"].ToString());
        ...
        private void btnDownload_OK_Click(object sender, EventArgs e)
        {
            ...
            if (txtStatementsFolder.Text != "")
            {
                ...
                {
                    var docFiles = new DirectoryInfo(txtStatementsFolder.Text).GetFiles("*.docx");
                        ...
                        while (true)
                        {
                            ...
                            if (j > 0)
                            {
                                if (sStatementFile.Length > 0)
                                {
                                    ...
                                    clsCustodyFees_Recs CustodyFees_Rec = new clsCustodyFees_Recs();
                                    CustodyFees_Rec.Record_ID = Convert.ToInt32(fgList[j, 18]);
                                    CustodyFees_Rec.GetRecord();
                                    CustodyFees_Rec.Statement_File = sStatementFile;
                                    CustodyFees_Rec.EditRecord();
                        ...
                        while (true)
                        {
                            ...
                            if (j > 0)
                            {
                                if (sMiscFile.Length > 0)
                                {
                                    ...
                                    CustodyFees_Rec.Record_ID = Convert.ToInt32(fgList[j, 18]);
                                    CustodyFees_Rec.GetRecord();
                                    CustodyFees_Rec.Misc_File = sMiscFile;
                                    CustodyFees_Rec.EditRecord();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            clsCustodyFees_Recs CustodyFees_Rec = new clsCustodyFees_Recs();
            CustodyFees_Rec.Record_ID = Convert.ToInt32(fgList[fgList.Row, 18]);
            CustodyFees_Rec.GetRecord();
            CustodyFees_Rec.Invoice_File = txtInvoice.Text;
            CustodyFees_Rec.Statement_File = txtStatement.Text;
            CustodyFees_Rec.Misc_File = txtMisc.Text;
            CustodyFees_Rec.EditRecord();
