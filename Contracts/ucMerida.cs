...
namespace Contracts
{
    public partial class ucMerida : UserControl
    {
        ...
        private void ucMerida_Load(object sender, EventArgs e)
        {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (sDescription.Length > 0)
            {
                ...
                Merida_Data = JsonConvert.DeserializeObject<MeridaData>(tokens[0]);
                lblOldMerida.Text = Merida_Data.old_Merida;

                Merida_Data = JsonConvert.DeserializeObject<MeridaData>(tokens[1]);
                txtNewMerida.Text = Merida_Data.new_Merida;

                string[] bokens = tokens[2].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    Merida_Data = JsonConvert.DeserializeObject<MeridaData>(bokens[i]);
                    if (Merida_Data.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + Merida_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = Merida_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + Merida_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void txtNewMerida_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
            CreateDescription();
        }
        ...
        private void CreateDescription()
        {
            ...
            if (txtNewMerida.Text.Trim() == "") lblStatus_Grp2.Visible = true;
            ...
            sDescription = "{'old_Merida' : '" + lblOldMerida.Text + "'}~{'new_Merida' : '" + txtNewMerida.Text + "'}~" + sTemp;
        }
        public string Description { get { return this.sDescription; } set { this.sDescription = value; } }
        public class MeridaData
        {
            public string old_Merida { get; set; }
            public string new_Merida { get; set; }
