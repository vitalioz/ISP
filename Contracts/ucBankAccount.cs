...
namespace Contracts
{
    public partial class ucBankAccount : UserControl
    {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            //------- define cmbBanks list ----------------------------
            cmbBanks.DataSource = Global.dtBanks.Copy();
            cmbBanks.DisplayMember = "Title";
            cmbBanks.ValueMember = "ID";

            //------- define cmbCurrencies list -----------------------
            ...
            if (sDescription.Length > 0)
            {
                ...
                txtAccNumber.Text = Bank_AccData.acc_number;

                Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(tokens[1]);
                cmbCurrencies.Text = Bank_AccData.currency;

                Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(tokens[2]);
                cmbType.SelectedIndex = Bank_AccData.type;

                Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(tokens[3]);
                txtOwners.Text = Bank_AccData.acc_number;
                ...
                else txtOwners.Enabled = true;

                Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(tokens[4]);
                lnkEmail.Text = Bank_AccData.source_email.Trim();

                Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(tokens[5]);
                cmbBanks.SelectedValue = Bank_AccData.bank_id;

                string[] bokens = tokens[6].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    Bank_AccData = JsonConvert.DeserializeObject<BankAccData>(bokens[i]);
                    if (Bank_AccData.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + Bank_AccData.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = Bank_AccData.file_name + "\t" + sTemp + "\t" + "0" + "\t" + Bank_AccData.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void lnkEmail_LinkClicked_1(object sender, LinkLabelLinkClickedEventArgs e)
        {
            Global.DMS_ShowFile("Customers\\" + sClientName, lnkEmail.Text);
        }
        ...
        private void txtAccNumber_LostFocus(object sender, EventArgs e)
        {
            CreateDescription();
        }
        private void txtOwners_LostFocus(object sender, EventArgs e)
        {
            CreateDescription();
        }
        private void CreateDescription()
        {
            if (bCheckList)
            {
                ...
                if (txtAccNumber.Text.Trim() != "" && Convert.ToInt32(cmbBanks.SelectedValue) != 0 && cmbCurrencies.Text != "" && lblDocsCount.Text != "") lblStatus.Text = "1";
                ...
                sDescription = "{'acc_number' : '" + txtAccNumber.Text + "'}~{'currency' : '" + cmbCurrencies.Text + "'}~" +
                               "{'type' : '" + cmbType.SelectedIndex + "'}~{'owners' : '" + txtOwners.Text + "'}~{'source_email' : '" + lnkEmail.Text + "'}~" +
                               "{'bank_id' : '" + cmbBanks.SelectedValue + "'}~" + sTemp;
            }
        }
