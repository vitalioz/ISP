...
namespace Contracts
{
    public partial class ucAMKA : UserControl
    {
        ...
        private void ucAMKA_Load(object sender, EventArgs e)
        {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (sDescription.Length > 0)
            {
                ...
                AMKA_Data = JsonConvert.DeserializeObject<AMKAData>(tokens[0]);
                lblOldAMKA.Text = AMKA_Data.old_AMKA;

                AMKA_Data = JsonConvert.DeserializeObject<AMKAData>(tokens[1]);
                txtNewAMKA.Text = AMKA_Data.new_AMKA;

                string[] bokens = tokens[2].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    AMKA_Data = JsonConvert.DeserializeObject<AMKAData>(bokens[i]);
                    if (AMKA_Data.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + AMKA_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = AMKA_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + AMKA_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void txtNewAMKA_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
            CreateDescription();
        }
        ...
        private void CreateDescription()
        {
            ...
            if (txtNewAMKA.Text.Trim() == "") lblStatus_Grp2.Visible = true;
            ...
            sDescription = "{'old_AMKA' : '" + lblOldAMKA.Text + "'}~{'new_AMKA' : '" + txtNewAMKA.Text + "'}~" + sTemp;
        }
        public string Description { get { return this.sDescription; } set { this.sDescription = value; } }
        public class AMKAData
        {
            public string old_AMKA { get; set; }
            public string new_AMKA { get; set; }
