...
namespace Contracts
{
    public partial class ucCountryTaxes : UserControl
    {
        ...
        private void ucCountryTaxes_Load(object sender, EventArgs e)
        {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (sDescription.Length > 0)
            {
                ...
                lblOldCountry.Text = Country_Data.old_country;
                ...
                cmbNewCountry.SelectedValue = Country_Data.new_country_id;
                ...
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    ...
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + Country_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = Country_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + Country_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void CreateDescription()
        {
            ...
            if (cmbNewCountry.Text.Trim() == "") lblStatus_Grp2.Visible = true;
            ...
            sTemp = "{'new_country_id' : '" + cmbNewCountry.SelectedValue + "'}~";
            for (i = 1; i < fgDocs.Rows.Count; i++)
                sTemp = sTemp + "{ 'file_name' : '" + fgDocs[i, "File_Name"] + "','file_type' : " + fgDocs[i, "DocType_ID"] + "}^";

            sTemp = sTemp + "~";

            sDescription = "{'old_country' : '" + lblOldCountry.Text + "'}~{'source_email' : '" + lnkEmail.Text + "'}~" + sTemp;
        }
