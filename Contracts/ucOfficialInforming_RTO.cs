...
namespace Contracts
{
    public partial class ucOfficialInforming_RTO : UserControl
    {
        DataColumn dtCol;
        DataRow dtRow;
        DataTable dtInform;
        ...
        public ucOfficialInforming_RTO()
        {
            InitializeComponent();
            EmptyContractData();
        }

        private void ucOfficialInforming_RTO_Load(object sender, EventArgs e)
        {
        ...
        private void tsbSend_Click(object sender, EventArgs e)
        {
            ...
            for (i = 1; i <= fgList.Rows.Count - 1; i++)
            {
                if (Convert.ToBoolean(fgList[i, 0]))
                {
                    ...
                    {
                        sBody = DailyEmailBody(Convert.ToInt32(fgList[i, "ClientTipos"]), Convert.ToInt32(fgList[i, "Service_ID"]));
                        ...
                        if (sOldCode != (fgList[i, "Code"] + ""))                     // if it's a new code write into Informings table record that will be send
                        {
                            ...
                            iRec_ID = Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 5, 6, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "ClientData"] + "",
                                               "backoffice@hellasfin.gr", "ΑΜΟΙΒΗ ΛΗΨΗΣ ΚΑΙ ΔΙΑΒΙΒΑΣΗΣ ΕΝΤΟΛΗΣ", sBody, fgList[i, "FileName"] + "", "", "", 0, 0, "");                        // 5 - e-mail 
                        }
                        ...
                        {
                            iRec_ID = Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 5, 6, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "ClientData"] + "",
                                               "backoffice@hellasfin.gr", "ΑΜΟΙΒΗ ΛΗΨΗΣ ΚΑΙ ΔΙΑΒΙΒΑΣΗΣ ΕΝΤΟΛΗΣ", sBody, fgList[i, "FileName"] + "", "", DateTime.Now.ToString(), 1, 1, "");   // 5 - e-mail  
                        }
                    ...
                    if (Convert.ToInt32(fgList[i, "ConnectionMethod"]) == 2)                  // 2 - post
                    {
                        ...
                        dtRow["f5"] = "ΘΕΜΑ: ΑΜΟΙΒΗ ΛΗΨΗΣ ΚΑΙ ΔΙΑΒΙΒΑΣΗΣ ΕΝΤΟΛΗΣ";
                        dtRow["f6"] = "Στα πλαίσια ενημέρωσής σας, αποστέλλουμε:" + "\n\n" + (Convert.ToInt32(fgList[i, "ClientTipos"]) == 2 ? " -	Τιμολόγιο παροχής υπηρεσιών" : "-	Απόδειξη παροχής υπηρεσιών") + "\n" +
                                      (Convert.ToInt32(fgList[i, "Service_ID"]) == 3 ? "-	Ανάλυση αμοιβής διαβίβασης εντολής " : "-	Ανάλυση αμοιβής λήψης και διαβίβασης εντολής") + "\n" + "\n" +
                        ...
                        Global.AddInformingRecord(0, Convert.ToInt32(fgList[i, "ID"]), 8, 6, Convert.ToInt32(fgList[i, "Client_ID"]), Convert.ToInt32(fgList[i, "Contract_ID"]), fgList[i, "ClientData"] + "", "",
                                                  "ΑΜΟΙΒΗ ΛΗΨΗΣ ΚΑΙ ΔΙΑΒΙΒΑΣΗΣ ΕΝΤΟΛΗΣ", "", "", sAttachedFiles, DateTime.Now.ToString(), 1, 1, "");                        // 8 - post                      
                    }
        ...
        private void tsbPrint_Click(object sender, EventArgs e)
        {
            ...
            locReports.Params = Convert.ToDateTime(dFrom.Value).ToString("dd/MM/yyyy") + " - " + Convert.ToDateTime(dTo.Value).ToString("dd/MM/yyyy") + "~" +
                            cmbProviders.Text + "~" + ucCS.txtContractTitle.Text + "~" + Global.UserName + "~" + "Τιμολόγια RTO";
        ...
        private void DefineCommandsList()
        {
            ...
            clsSettlements klsSettlements = new clsSettlements();
            klsSettlements.DateInsFrom = dFrom.Value;
            klsSettlements.DateInsTo = dTo.Value;
            klsSettlements.StockCompany_ID = Convert.ToInt32(cmbProviders.SelectedValue);
            klsSettlements.ClientCode = lblCode.Text;
            klsSettlements.GetInvoicesList();

            i = 0;
            foreach (DataRow dtRow in klsSettlements.List.Rows)
            {
                if ((dtRow["Invoice_Num"] + "") != "")
                {
                    ...
                    if (Convert.ToInt32(dtRow["Contract_ConnectionMethod"]) == 1)
                    {
                        ...
                        sConnectionData = dtRow["Contract_EMail"] + "";
                    }
                    ...
                    {
                        sClientName = (dtRow["Surname"] + "").Trim();
                    ...
                    if (Convert.ToInt32(dtRow["Contract_ConnectionMethod"]) == 2)
                    {
                        sConnectionMethod = "Ταχ/κη αποστολή";
                        sConnectionData = sRecipientName + "\n" + dtRow["Contract_Address"] + "\n" + dtRow["Contract_City"] + " " + dtRow["Contract_ZIP"];
                        if (dtRow["CountryTitleEn"] + "" != "Greece") sConnectionData = sConnectionData + " " + dtRow["CountryTitleEn"];
                    }

                    fgList.AddItem(false + "\t" + i + "\t" + sClientName + "\t" + dtRow["ContractTitle"] + "\t" + dtRow["Code"] + "\t" + dtRow["Portfolio"] + "\t" +
                                  dtRow["ServiceTitle"] + "\t" + dtRow["Invoice_Num"] + "\t" + dtRow["Invoice_DateIns"] + "\t" + dtRow["RTO_CompanyFees"] + "\t" +
                                  ...
                                  dtRow["ClientTipos"] + "\t" + dtRow["StockCompany_ID"] + "\t" + dtRow["Invoice_Titles_ID"] + "\t" + dtRow["Service_ID"] + "\t" +
        ...
        private string DailyEmailBody(int iClientTipos, int iService_ID)
        {
            string sBody;
            ...
            "<center> ΘΕΜΑ: ΑΜΟΙΒΗ ΛΗΨΗΣ ΚΑΙ ΔΙΑΒΙΒΑΣΗΣ ΕΝΤΟΛΗΣ </center>" + "<br/><br/><br/><br/><br/>" +
            ...
            (iService_ID == 3 ? " - Ανάλυση αμοιβής διαβίβασης εντολής " : " - Ανάλυση αμοιβής λήψης και διαβίβασης εντολής") +
        ...
        private void mnuCommandData_Click(object sender, EventArgs e)
        {
            ...
            locOrderSecurity.Rec_ID = Convert.ToInt32(fgList[fgList.Row, 14]);
