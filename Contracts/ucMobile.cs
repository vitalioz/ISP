...
namespace Contracts
{
    public partial class ucMobile : UserControl
    {
        ...
        private void ucMobile_Load(object sender, EventArgs e)
        {
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (sDescription.Length > 0)
            {
                ...
                Mobile_Data = JsonConvert.DeserializeObject<MobileData>(tokens[0]);
                lblOldNumber.Text = Mobile_Data.old_number.Trim();

                Mobile_Data = JsonConvert.DeserializeObject<MobileData>(tokens[1]);
                lnkEmail.Text = Mobile_Data.source_email.Trim();

                Mobile_Data = JsonConvert.DeserializeObject<MobileData>(tokens[2]);
                if (Mobile_Data.owner.Trim() == "1") rbOwner_Yes.Checked = true;
                else rbOwner_Yes.Checked = false;

                Mobile_Data = JsonConvert.DeserializeObject<MobileData>(tokens[3]);
                txtNewNumber.Text = Mobile_Data.new_number.Trim();

                string[] bokens = tokens[4].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    Mobile_Data = JsonConvert.DeserializeObject<MobileData>(bokens[i]);
                    if (Mobile_Data.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + Mobile_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = Mobile_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + Mobile_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
