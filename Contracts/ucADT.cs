...
namespace Contracts
{
    public partial class ucADT : UserControl
    {
        ...
        private void ucADT_Load(object sender, EventArgs e)
        {
        }
        ...
        public void StartInit(int iStatus, string sCN, int iKlient_ID, int iDefaultDocType1_ID, int iDocDefaultType2_ID)
        {
            ...
            if (iStatus == 0) grp1.Visible = false;
            else grp1.Visible = true;
            ...
            if (sDescription.Length > 0)
            {
                ...
                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[0]);
                lblOldNumber.Text = ADT_Data.old_number;

                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[1]);
                lblOldPolice.Text = ADT_Data.old_police;

                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[2]);
                lblOldExpireDate.Text = ADT_Data.old_expiredate;

                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[3]);
                txtNewNumber.Text = ADT_Data.new_number;

                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[4]);
                txtNewPolice.Text = ADT_Data.new_police;

                ADT_Data = JsonConvert.DeserializeObject<ADTData>(tokens[5]);
                dNewExpireDate.Text = ADT_Data.new_expiredate;

                string[] bokens = tokens[6].Split('^');
                for (i = 0; i < bokens.Length - 1; i++)
                {
                    ADT_Data = JsonConvert.DeserializeObject<ADTData>(bokens[i]);
                    if (ADT_Data.file_name.Length > 0)
                    {
                        sTemp = "";
                        foundRows = Global.dtDocTypes.Select("ID = " + ADT_Data.file_type);
                        if (foundRows.Length > 0) sTemp = foundRows[0]["Title"] + "";
                        sTemp = ADT_Data.file_name + "\t" + sTemp + "\t" + "0" + "\t" + ADT_Data.file_type + "\t" + "" + "\t" + "1";        // 1 - Status - file exists, but isn't confirmed
        ...
        private void txtNewPolice_LostFocus(object sender, EventArgs e)
        {
            CreateDescription();
        }
        ...
        private void CreateDescription()
        {
            ...
            if (txtNewNumber.Text.Trim() == "" || txtNewPolice.Text.Trim() == "" || dNewExpireDate.Value < Convert.ToDateTime("2000/01/01")) lblStatus_Grp2.Visible = true;
