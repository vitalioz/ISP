...
namespace Custody
{
    public partial class frmExecutionFiles2 : Form

    {
        DataTable dtList, dtCompare, dtProvidersRecs, dtChildOrders, dtDepositories_Alias;
        DataView dtView;
        DataColumn dtCol;
        DataRow dtRow, dtRow1;
        DataRow[] foundRows;
        int i, j, iRow, iOdd, iStockExchange_ID, iDepository_ID, iCommand_ID, iCommand_Executions_ID, iRightsLevel, iMaxQuantity_Command_ID;
        ...
        clsCommandsExecutionsDetails CommandsExecutionsDetails2 = new clsCommandsExecutionsDetails();
        public frmExecutionFiles2()
        {
            InitializeComponent();
        ...
        private void frmExecutionFiles2_Load(object sender, EventArgs e)
        {
            bCheckList = false;
            bCheckCompare = false;
            ...
            csOdd = fgMatching.Styles.Add("Odd");
            csOdd.BackColor = Color.LightGray;

            csFinish = fgMatching.Styles.Add("Finish");
            ...
            //------- fgMatching ----------------------------
            fgMatching.SelectionMode = C1.Win.C1FlexGrid.SelectionModeEnum.Cell;
            fgMatching.Styles.ParseString("Focus{Font:Microsoft Sans Serif, 8.25pt, style=Bold; BackColor:LightBlue; ForeColor:Black;} Highlight{Font:Microsoft Sans Serif, 8.25pt, style=Bold; BackColor:LightBlue; ForeColor:Black;}");
            fgMatching.BeforeEdit += new C1.Win.C1FlexGrid.RowColEventHandler(fgMatching_BeforeEdit);
            fgMatching.AfterEdit += new C1.Win.C1FlexGrid.RowColEventHandler(fgMatching_AfterEdit);
            fgMatching.CellChanged += fgMatching_CellChanged;
            fgMatching.OwnerDrawCell += fgMatching_OwnerDrawCell;
            fgMatching.RowColChange += new EventHandler(fgMatching_RowColChange);
            fgMatching.DrawMode = DrawModeEnum.OwnerDraw;
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            tcBusinessTypes.Width = tabMain.Width - 36;
            fgList.Width = tabMain.Width - 36;
            fgList.Height = tabMain.Height - 88;

            fgMatching.Height = tabMain.Height - 64;

            fgProvider2.Width = tabMain.Width - fgMatching.Width - 24;
            ...
            fgList2.Width = tabMain.Width - fgMatching.Width - 24;
        ...
        private void DefineList()
        {
            ...
            if (Convert.ToInt32(cmbServiceProviders.SelectedValue) != 0)
            {
                ...
                CommandsExecutionsDetails.StockCompany_ID = Convert.ToInt32(cmbServiceProviders.SelectedValue);
                CommandsExecutionsDetails.DateFrom = dAktionDate.Value;
                CommandsExecutionsDetails.DateTo = dAktionDate.Value;
                CommandsExecutionsDetails.GetExecutedCommands();
                ...
                foreach (DataRow dtRow in CommandsExecutionsDetails.List.Rows)
                {
                    if (Convert.ToDateTime(dtRow["ExecuteDate"]) != Convert.ToDateTime("1900/01/01"))
                    {
                        ...
                        fgList.AddItem(i + "\t" + dtRow["BulkCommand"] + "\t" + dtRow["ClientFullName"] + "\t" + dtRow["ContractTitle"] + "\t" + "" + "\t" +
                                       dtRow["Code"] + "\t" + dtRow["Portfolio"] + "\t" + (Convert.ToInt16(dtRow["Aktion"]) == 1 ? "BUY" : "SELL") + "\t" +
                                       dtRow["Share_Title"] + "\t" + dtRow["Share_Code"] + "\t" + dtRow["ISIN"] + "\t" +
                                       Convert.ToDecimal(dtRow["RealPrice"]).ToString("0.######") + "\t" + Convert.ToDecimal(dtRow["RealQuantity"]).ToString("0.######") +
                                       "\t" + Convert.ToDecimal(dtRow["RealAmount"]).ToString("0.##") + "\t" + dtRow["Currency"] + "\t" + dtRow["SE_Code"] + "\t" +
                                       dtRow["Depository_Code"] + "\t" + dtRow["FeesDiff"] + "\t" + dtRow["FeesMarket"] + "\t" + dtRow["AccruedInterest"] + "\t" +
                                       dtRow["Commission"] + "\t" + dtRow["ClientOrder_ID"] + "\t" + dtRow["ID"] + "\t" + dtRow["Command_ID"]);
                    }
                }
                fgList.Redraw = true;
            }
        ...
        private void tsbFinish_Click(object sender, EventArgs e)
        {
            ...
            dtProvidersRecs = new DataTable("ProvidersRecs");
            dtCol = dtProvidersRecs.Columns.Add("StockCompany_ID", System.Type.GetType("System.Int32"));
            dtCol = dtProvidersRecs.Columns.Add("TradeDate", System.Type.GetType("System.DateTime"));
            dtCol = dtProvidersRecs.Columns.Add("TradeTime", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("SettlementDate", System.Type.GetType("System.DateTime"));
            dtCol = dtProvidersRecs.Columns.Add("Command_ID", System.Type.GetType("System.Int32"));
            dtCol = dtProvidersRecs.Columns.Add("Aktion", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Code", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Portfolio", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("ClientName", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Contract_ID", System.Type.GetType("System.Int32"));
            dtCol = dtProvidersRecs.Columns.Add("ContractTitle", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("SecurityCode", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("ISIN", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("SecurityDescription", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Quantity", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("Price", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("TradeCurrency", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("AccruedInterest", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("MarketFee", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("StockExchange_ID", System.Type.GetType("System.Int32"));
            dtCol = dtProvidersRecs.Columns.Add("StockExchange_Code", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Depository_ID", System.Type.GetType("System.Int32"));
            dtCol = dtProvidersRecs.Columns.Add("Depository_Code", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("SettlementCurrency", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Notes", System.Type.GetType("System.String"));
            dtCol = dtProvidersRecs.Columns.Add("Fee", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("Commission", System.Type.GetType("System.Decimal"));
            dtCol = dtProvidersRecs.Columns.Add("RefNumber", System.Type.GetType("System.String"));

            for (j = 1; j <= fgMatching.Rows.Count - 1; j = j + 2)
            {
                if (Convert.ToBoolean(fgMatching[j + 1, "Check"]) && Convert.ToInt16(fgMatching[j + 1, "Status"]) == 0)
                {
                    sRefNumber = fgMatching[j, "ClientOrder_ID"] + "";

                    //if (sRefNumber == "590239320970")
                    //    j = j;

                    iCommand_ID = Convert.ToInt32(fgMatching[j + 1, "ID"]);                          // j + 1 - it's ISP Data 

                    iStockExchange_ID = Convert.ToInt32(fgMatching[j, "SE_ID"]);                     // j - it's Provider Data
                    sSE_ReutersCode = "";
                    foundRows = Global.dtStockExchanges.Select("ID = " + iStockExchange_ID);
                    if (foundRows.Length > 0)
                        sSE_ReutersCode = foundRows[0]["ReutersCode"] + "";

                    iDepository_ID = Convert.ToInt32(fgMatching[j, "Depository_ID"]);
                    sDepository_Code = fgMatching[j, "Depository"] + "";                             // j - it's Provider Data
                    ...
                    decQuantityNominal = Convert.ToDecimal(fgMatching[j, "Quantity"]);                // j - it's Provider Data
                    decAccrued = Convert.ToDecimal(fgMatching[j, "Accrued"]);                         // j - it's Provider Data
                    decMarketFee = Convert.ToDecimal(fgMatching[j, "MarketFee"]);                     // j - it's Provider Data
                    decFee = Convert.ToDecimal(fgMatching[j, "Fee"]);                                 // j - it's Provider Data
                    decCommission = Convert.ToDecimal(fgMatching[j, "Commission"]);                   // j - it's Provider Data
                    ...
                    Orders.FeesDiff = Convert.ToDecimal(fgMatching[j, "Fee"]);                       // j - it's Provider Data
                    Orders.FeesMarket = Convert.ToDecimal(fgMatching[j, "MarketFee"]);               // j - it's Provider Data    
                    Orders.AccruedInterest = Convert.ToDecimal(fgMatching[j, "Accrued"]);            // j - it's Provider Data
                    Orders.Commission = Convert.ToDecimal(fgMatching[j, "Commission"]);              // j - it's Provider Data     
                    Orders.EditRecord();
                    */
                    ...
                    if (decQuantityNominal != 0)
                    {
                        foreach (DataRow dtRow in CommandsExecutionsDetails.List.Rows)
                        {
                            if (dtRow["ClientOrder_ID"] + "" == fgMatching[j, "ClientOrder_ID"] + "")
                            {
                                ...
                                sProductType = "";
                                switch (Convert.ToInt32(Orders.Product_ID))
                                {
                                ...
                                foundRows = dtProvidersRecs.Select("RefNumber = '" + sRefNumber + "' AND Command_ID = " + Orders.Record_ID);
                                if (foundRows.Length == 0)
                                {
                                    dtRow1 = dtProvidersRecs.NewRow();
                                    dtRow1["StockCompany_ID"] = Convert.ToInt32(cmbServiceProviders.SelectedValue);
                                    dtRow1["TradeDate"] = dTradeDate;
                                    dtRow1["TradeTime"] = sTradeTime;
                                    dtRow1["SettlementDate"] = dSettlementDate;
                                    dtRow1["Command_ID"] = Convert.ToInt32(dtRow["Command_ID"]);
                                    dtRow1["Aktion"] = Orders.Aktion == 1 ? "BUY" : "SELL";
                                    dtRow1["Code"] = Orders.Code + "";
                                    dtRow1["Portfolio"] = Orders.ProfitCenter + "";
                                    dtRow1["ClientName"] = Orders.ClientName + "";
                                    dtRow1["Contract_ID"] = Orders.Contract_ID;
                                    dtRow1["ContractTitle"] = Orders.ContractTitle + "";
                                    dtRow1["SecurityCode"] = sProductType;
                                    dtRow1["ISIN"] = Orders.Security_ISIN + "";
                                    dtRow1["SecurityDescription"] = Orders.Security_Title + "";
                                    dtRow1["Quantity"] = Convert.ToDecimal(dtRow["RealQuantity"]);
                                    dtRow1["Price"] = Orders.RealPrice;
                                    dtRow1["TradeCurrency"] = dtRow["Currency"] + "";
                                    dtRow1["AccruedInterest"] = decKoef * decAccrued;
                                    dtRow1["MarketFee"] = decKoef * decMarketFee;
                                    dtRow1["StockExchange_ID"] = iStockExchange_ID;
                                    dtRow1["StockExchange_Code"] = sSE_ReutersCode;                      //dtRow1["SE_Code"] + "";
                                    dtRow1["Depository_ID"] = iDepository_ID;
                                    dtRow1["Depository_Code"] = sDepository_Code;
                                    dtRow1["SettlementCurrency"] = dtRow["Currency"] + "";
                                    dtRow1["Notes"] = Orders.Notes;
                                    dtRow1["Fee"] = decKoef * decFee;
                                    dtRow1["Commission"] = locCommission;
                                    dtRow1["RefNumber"] = sRefNumber;
                                    dtProvidersRecs.Rows.Add(dtRow1);
                                }
                                else
                                {
                                    foundRows[0]["Quantity"] = Convert.ToDecimal(foundRows[0]["Quantity"]) + Convert.ToDecimal(dtRow["RealQuantity"]);
                                    foundRows[0]["MarketFee"] = Convert.ToDecimal(foundRows[0]["MarketFee"]) + Convert.ToDecimal(decKoef * decMarketFee);
                                    foundRows[0]["Fee"] = Convert.ToDecimal(foundRows[0]["Fee"]) + Convert.ToDecimal(decKoef * decFee);
                                }
                            }
                        }
                    }
            ...
            foreach (DataRow dtRow in dtProvidersRecs.Rows)
            {
                Orders_ProvidersRecs = new clsOrders_ProvidersRecs();
                Orders_ProvidersRecs.StockCompany_ID = Convert.ToInt32(dtRow["StockCompany_ID"]);
                Orders_ProvidersRecs.TradeDate = Convert.ToDateTime(dtRow["TradeDate"]);
                Orders_ProvidersRecs.TradeTime = dtRow["TradeTime"] + "";
                Orders_ProvidersRecs.SettlementDate = Convert.ToDateTime(dtRow["SettlementDate"]);
                Orders_ProvidersRecs.CompanyCode = sEffectCode;
                Orders_ProvidersRecs.Command_ID = Convert.ToInt32(dtRow["Command_ID"]);
                ...
                Orders_ProvidersRecs.ClientName = dtRow["ClientName"] + "";
                Orders_ProvidersRecs.ContractTitle = dtRow["ContractTitle"] + "";
                Orders_ProvidersRecs.SecurityCode = dtRow["SecurityCode"] + "";
                Orders_ProvidersRecs.ISIN = dtRow["ISIN"] + "";
                Orders_ProvidersRecs.SecurityDescription = dtRow["SecurityDescription"] + "";
                Orders_ProvidersRecs.Quantity = Convert.ToDecimal(dtRow["Quantity"]);
                Orders_ProvidersRecs.Price = Convert.ToDecimal(dtRow["Price"]);
                Orders_ProvidersRecs.TradeCurrency = dtRow["TradeCurrency"] + "";
                Orders_ProvidersRecs.AccruedInterest = Convert.ToDecimal(dtRow["AccruedInterest"]);
                Orders_ProvidersRecs.MarketFee = Convert.ToDecimal(dtRow["MarketFee"]);
                Orders_ProvidersRecs.StockExchange_ID = Convert.ToInt32(dtRow["StockExchange_ID"]);
                Orders_ProvidersRecs.StockExchange_Code = dtRow["StockExchange_Code"] + "";
                Orders_ProvidersRecs.Depository_ID = Convert.ToInt32(dtRow["Depository_ID"]);
                Orders_ProvidersRecs.Depository_Code = dtRow["Depository_Code"] + ""; ;
                Orders_ProvidersRecs.SettlementCurrency = dtRow["SettlementCurrency"] + "";
                Orders_ProvidersRecs.CurrencyRate = 1;
                Orders_ProvidersRecs.Notes = dtRow["Notes"] + "";
                Orders_ProvidersRecs.Fee = Convert.ToDecimal(dtRow["Fee"]);
                Orders_ProvidersRecs.Commission = Convert.ToDecimal(dtRow["Commission"]);
                Orders_ProvidersRecs.RefNumber = dtRow["RefNumber"] + ""; ;
                ...
                case 17:                                                           //------------------------------17 - PIREAUS SECURITIES
                    ...
                    if (locImportData.Aktion == 1)
                    {
                        ...
                        foreach (DataRow dtRow in dtList.Rows)
                        {
                            ...
                            CustodyCommands.Commission = Convert.ToDecimal("0" + dtRow["f12"]);
                            CustodyCommands.Fees = Convert.ToDecimal("0" + dtRow["f13"]);
                            CustodyCommands.MarketFee = Convert.ToDecimal("0" + dtRow["f14"]);
                            CustodyCommands.Taxes = Convert.ToDecimal("0" + dtRow["f15"]);
                            CustodyCommands.SettlementAmount = Convert.ToDecimal("0" + dtRow["f16"]);
                            CustodyCommands.ExchangeRate = 0;
                            CustodyCommands.SettlementAmountCurr = Convert.ToDecimal("0" + dtRow["f17"]);
                ...
                case 20:                                                           //------------------------------ 20 - BNP ARBITRAGE
                    ...
                    if (locImportData.Aktion == 1)
                    {
                        ...
                        foreach (DataRow dtRow in dtList.Rows)
                        {
                            ...
                            CustodyCommands.Sign = dtRow["f7"] + "" == "Buy" ? "BUY" : "SELL";
        ...
        private void Matching()
        {
            ...
            dtCol = dtCompare.Columns.Add("Details_ID_1", System.Type.GetType("System.Int32"));
            dtCol = dtCompare.Columns.Add("Details_ID_2", System.Type.GetType("System.Int32"));
            ...
            for (i = 1; i <= fgProvider.Rows.Count - 1; i++)
            {
                ...
                if (foundRows.Length == 0)
                {
                    ...
                    dtRow["ID_2"] = 0;
                    dtRow["Details_ID_1"] = 0;
                    dtRow["Details_ID_2"] = 0;
                    dtRow["Status_1"] = 0;
                    dtRow["Status_2"] = 0;
                ...
                else
                {
                    ...
                    if (Convert.ToInt32(cmbServiceProviders.SelectedValue) != 17)
                    {                                                         // IS NOT 17 - PIREAUS SECURITIES
            ...
            foreach (DataRow dtRow1 in CommandsExecutionsDetails.List.Rows)
            {
                ...
                if (foundRows.Length == 0)
                {
                    ...
                    dtRow["ID_1"] = 0;
                    dtRow["ID_2"] = Convert.ToInt32(dtRow1["Command_ID"]);
                    dtRow["Details_ID_1"] = 0;
                    dtRow["Details_ID_2"] = Convert.ToInt32(dtRow1["ID"]);
                    dtRow["Status_1"] = 0;
                ...
                else
                {
                    ...
                    foundRows[0]["ID_2"] = Convert.ToInt32(dtRow1["Command_ID"]);
                    foundRows[0]["Details_ID_2"] = Convert.ToInt32(dtRow1["ID"]);
            ...
            foreach (DataRow dtRow in dtCompare.Rows)
            {
                ...
                fgMatching.AddItem((Convert.ToInt32(dtRow["Check"]) == 0 ? false : true) + "\t" + dtRow["ClientOrder_ID"] + "\t" + cmbServiceProviders.Text + "\t" +
                                  ...
                                  Convert.ToDecimal(dtRow["Commission_1"]).ToString("0.######") + "\t" + dtRow["ID_1"] + "\t" + dtRow["Details_ID_1"] + "\t" +
                                  dtRow["Status_1"] + "\t" + iOdd + "\t" + dtRow["TradeTime"] + "\t" + dtRow["AktionDate_1"] + "\t" + dtRow["SettlementDate"] + "\t" +
                                  dtRow["SE_ID_1"] + "\t" + dtRow["Depository_ID"]);

                if (Convert.ToDecimal(dtRow["Quantity_2"]) != 0)
                    dtRow["Price_2"] = Convert.ToDecimal(dtRow["Amount_2"]) / Convert.ToDecimal(dtRow["Quantity_2"]);
                fgMatching.AddItem((Convert.ToInt32(dtRow["Check"]) == 0 ? false : true) + "\t" + dtRow["ClientOrder_ID"] + "\t" + "HellasFin" + "\t" +
                                  ...
                                  Convert.ToDecimal(dtRow["Commission_2"]).ToString("0.######") + "\t" + dtRow["ID_2"] + "\t" + dtRow["Details_ID_2"] + "\t" +
                                  dtRow["Status_2"] + "\t" + iOdd + "\t" + dtRow["TradeTime"] + "\t" + dtRow["AktionDate_2"] + "\t" + dtRow["SettlementDate"] + "\t" +
                                  dtRow["SE_ID_2"] + "\t" + dtRow["Depository_ID"]);
            }
            fgMatching.AllowMerging = C1.Win.C1FlexGrid.AllowMergingEnum.Free;
            // Merge values in columns 1. 
            fgMatching.Cols[1].AllowMerging = true;
            fgMatching.Redraw = true;
        }
        private void fgMatching_RowColChange(object sender, EventArgs e)
        {
            if (bCheckCompare)
            {
                if (fgMatching.Row > 0)
                {
                    ...
                    foreach (DataRow dtRow in CustodyCommands.List.Rows)
                    {
                        if ((fgMatching[fgMatching.Row, 1] + "") == (dtRow["RefNumber"] + ""))
                        {
                            i = i + 1;
                    ...
                    DefineChildOrders(fgMatching[fgMatching.Row, 1] + "", dAktionDate.Value);
                    ...
                    fgList2.Rows.Count = 1;
                    foreach (DataRow dtRow in CommandsExecutionsDetails.List.Rows) {
                        if ((fgMatching[fgMatching.Row, 1] + "") == (dtRow["ClientOrder_ID"] + "")) {
                            i = i + 1;
                            fgList2.AddItem(i + "\t" + (Convert.ToInt32(dtRow["Aktion"]) == 1 ? "BUY" : "SELL") + "\t" + dtRow["RealQuantity"] + "\t" + dtRow["RealPrice"] + "\t" + 
                                dtRow["FeesDiff"] + "\t" + dtRow["FeesMarket"] + "\t" + dtRow["AccruedInterest"] + "\t" + dtRow["Commission"] + "\t" +
                                dtRow["Currency"] + "\t" + dtRow["ContractTitle"] + "\t" + dtRow["Code"] + "\t" + dtRow["Portfolio"] + "\t" +
                                dtRow["ClientFullName"] + "\t" + dtRow["BulkCommand"] + "\t" + dtRow["Share_Title"] + "\t" + dtRow["Share_Code"] + "\t" + dtRow["ISIN"] + "\t" +
                                dtRow["SE_Code"] + "\t" + dtRow["AktionDate"] + "\t" + dtRow["Notes"] + "\t" + dtRow["ID"] + "\t" + dtRow["CommandType_ID"]);
                            ...
                            Orders = new clsOrdersSecurity();
                            Orders.AktionDate = dAktionDate.Value;
                            ...
                            foreach (DataRow dtRow1 in Orders.List.Rows) {
                                switch (Convert.ToInt32(dtRow1["CommandType_ID"])) {
                                    case 1:
                                        fgList2.AddItem("" + "\t" + "" + "\t" + dtRow1["RealQuantity"] + "\t" + dtRow1["RealPrice"] + "\t" +
                                                dtRow1["FeesDiff"] + "\t" + dtRow1["FeesMarket"] + "\t" + dtRow1["AccruedInterest"] + "\t" + dtRow1["Commission"] + "\t" +
                                                dtRow1["Currency"] + "\t" + dtRow1["ContractTitle"] + "\t" + dtRow1["Code"] + "\t" + dtRow1["Portfolio"] + "\t" +
                                                dtRow1["ClientFullName"] + "\t" + dtRow1["BulkCommand"] + "\t" + dtRow1["Share_Title"] + "\t" + dtRow1["Share_Code"] + "\t" + dtRow1["ISIN"] + "\t" +
                                                dtRow1["SE_Code"] + "\t" + dtRow1["AktionDate"] + "\t" + dtRow1["Notes"] + "\t" + dtRow1["ID"] + "\t" + dtRow1["CommandType_ID"]);
                                        break;
                                    case 4:
                                        ...
                                        if (j >= 0) {
                                            ...
                                            foreach (DataRow dtRow2 in Orders4.List.Rows) {
                                                ...
                                                fgList2.AddItem("" + "\t" + sTemp + "\t" + dtRow2["RealQuantity"] + "\t" + dtRow2["RealPrice"] + "\t" +
                                                      dtRow2["FeesDiff"] + "\t" + dtRow2["FeesMarket"] + "\t" + dtRow2["AccruedInterest"] + "\t" + dtRow2["Commission"] + "\t" +
                                                      dtRow2["Currency"] + "\t" + dtRow2["ContractTitle"] + "\t" + dtRow2["Code"] + "\t" + dtRow2["Portfolio"] + "\t" +
                                                      dtRow2["ClientFullName"] + "\t" + dtRow2["BulkCommand"] + "\t" + dtRow2["Share_Title"] + "\t" + dtRow2["Share_Code"] + "\t" + dtRow2["ISIN"] + "\t" +
                                                      dtRow2["SE_Code"] + "\t" + dtRow2["AktionDate"] + "\t" + dtRow2["Notes"] + "\t" + dtRow2["ID"] + "\t" + dtRow2["CommandType_ID"]);
                                            }
                                        }
                                        break;
                                }
        ...
        private void fgMatching_BeforeEdit(object sender, RowColEventArgs e)
        {
            if (e.Col == 0)
            {
                if (Convert.ToInt32(fgMatching[fgMatching.Row, "Status"]) == 0)
                {
                    e.Cancel = false;
                    sTemp = fgMatching[fgMatching.Row, "ClientOrder_ID"] + "";

                    for (j = 1; j <= fgMatching.Rows.Count - 1; j++)
                        if (fgMatching[j, "ClientOrder_ID"] + "" == sTemp) fgMatching[j, 0] = fgMatching[fgMatching.Row, 0];
                }
                else e.Cancel = true;
            }
            else e.Cancel = true;
        }
        private void fgMatching_AfterEdit(object sender, RowColEventArgs e)
        {
            if (e.Col == 0) EditFlag(e.Row);
        }
        private void fgList_BeforeEdit(object sender, RowColEventArgs e)
        {
            if (e.Col == 21) e.Cancel = false;                                                      // 21 - ClientOrder_ID                               
            else e.Cancel = true;
        }
        private void fgList_AfterEdit(object sender, RowColEventArgs e)
        {
            if (e.Col == 21)                                                                        // 21 - ClientOrder_ID
            {
                CommandsExecutionsDetails2 = new clsCommandsExecutionsDetails();
                CommandsExecutionsDetails2.Record_ID = Convert.ToInt32(fgList[fgList.Row, "ID"]+"");
                CommandsExecutionsDetails2.GetRecord();
                CommandsExecutionsDetails2.SecondOrdID = fgList[fgList.Row, "ClientOrder_ID"] + "";
                CommandsExecutionsDetails2.EditRecord();
            }
        }
        private void DefineChildOrders(string sClientOrder_ID, DateTime dActionDate)
        {
            ...
            dtCol = dtChildOrders.Columns.Add("Command_ID", System.Type.GetType("System.Int32"));
        ...
        private void AddChildOrderRecord(DataRow dtRow)
        {
            ...
            dtRow1["ID"] = dtRow["ID"];
            dtRow1["Command_ID"] = dtRow["Command_ID"];
        ...
        {
            for (i = 1; i <= fgMatching.Rows.Count - 1; i++)
            {
                if (Convert.ToInt32(fgMatching[i, "Status"]) == 0)
                {
                    fgMatching[i, "Check"] = chkMatching.Checked;
                    EditFlag(i);
                }
            }
        }
        ...
        {
            if ((fgMatching[iRow, "Company"] + "").Trim() == (cmbServiceProviders.Text + "").Trim())
            {
                iRow = iRow + 1;
                fgMatching[iRow, 0] = fgMatching[iRow - 1, 0];
            }
            if ((fgMatching[iRow, "Company"] + "").Trim() != (cmbServiceProviders.Text + "").Trim())
            {
                fgMatching[iRow - 1, 0] = fgMatching[iRow, 0];

                if (Convert.ToBoolean(fgMatching[iRow - 1, 0]))
                {
                    fgMatching[iRow, "Fee"] = fgMatching[iRow - 1, "Fee"];
                    fgMatching[iRow, "MarketFee"] = fgMatching[iRow - 1, "MarketFee"];
                    fgMatching[iRow, "Accrued"] = fgMatching[iRow - 1, "Accrued"];
                    fgMatching[iRow, "Commission"] = fgMatching[iRow - 1, "Commission"];
                }
                else
                {
                    fgMatching[iRow, "Fee"] = 0;
                    fgMatching[iRow, "MarketFee"] = 0;
                    fgMatching[iRow, "Accrued"] = 0;
                    fgMatching[iRow, "Commission"] = 0;
                }
            }
        }
        private void fgMatching_CellChanged(object sender, RowColEventArgs e)
        {
            if (e.Row > 0)
            {
                if (e.Col == 17)
                {                                                                                              // 17 - Status of FinishAction
                    if (Convert.ToInt32(fgMatching[e.Row, "Status"]) == 1) fgMatching.Rows[e.Row].Style = csFinish;
                }
                if (e.Col == 18)
                {                                                                                              // 18 - StyleFlag
                    if (Convert.ToInt32(fgMatching[e.Row, "Status"]) != 1)
                        if (Convert.ToInt32(fgMatching[e.Row, "StyleFlag"]) == 1) fgMatching.Rows[e.Row].Style = csOdd;
                }
            }
        }
        private void fgMatching_OwnerDrawCell(object sender, OwnerDrawCellEventArgs e)
        {
            if (e.Row % 2 != 0)
            {                                                                                             //e.Row > 0 && e.Row < fgMatching.Rows.Count - 1 )

                if (e.Col == 3)                                                                                                // 3 - AktionDate
                    if (fgMatching[e.Row, 3].ToString() != fgMatching[e.Row + 1, 3].ToString()) e.Style = csDiff;

                if (e.Col == 4)                                                                                                // 4 - ISIN
                    if (fgMatching[e.Row, 4].ToString() != fgMatching[e.Row + 1, 4].ToString()) e.Style = csDiff;

                if (e.Col == 5)                                                                                                // 5 - Aktion
                    if (fgMatching[e.Row, 5].ToString() != fgMatching[e.Row + 1, 5].ToString()) e.Style = csDiff;

                if (e.Col == 6)                                                                                                // 6 - Currency
                    if (fgMatching[e.Row, 6].ToString() != fgMatching[e.Row + 1, 6].ToString()) e.Style = csDiff;

                if (e.Col == 7)                                                                                                // 7 - Quantity
                    if (Convert.ToDecimal(fgMatching[e.Row, 7]) != Convert.ToDecimal(fgMatching[e.Row + 1, 7])) e.Style = csDiff;

                if (e.Col == 8)                                                                                                // 8 - Price
                    if (Convert.ToDecimal(fgMatching[e.Row, 8]) != Convert.ToDecimal(fgMatching[e.Row + 1, 8])) e.Style = csDiff;

                if (e.Col == 9)                                                                                                // 9 - SE_Code
                    if (fgMatching[e.Row, 9].ToString() != fgMatching[e.Row + 1, 9].ToString()) e.Style = csDiff;

                //if (e.Col == 10)                                                                                              // 10 - Depository_Code
                //    if (fgMatching[e.Row, 10].ToString() != fgMatching[e.Row + 1, 10].ToString()) e.Style = csDiff;
            }
        }
